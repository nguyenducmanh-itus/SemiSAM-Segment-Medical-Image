...
def crop_and_concat(x1, x2):
    # x1 và x2 đều có dạng (B, C, D, H, W)
    D1, H1, W1 = x1.shape[2:]
    D2, H2, W2 = x2.shape[2:]

    # Tính lượng cần crop để trùng kích thước
    d_crop = (D2 - D1) // 2
    h_crop = (H2 - H1) // 2
    w_crop = (W2 - W1) // 2

    x2_cropped = x2[:, :, d_crop:D2-d_crop, h_crop:H2-h_crop, w_crop:W2-w_crop]
...
class unetUp(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        ...
        return self.conv(crop_and_concat(outputs1, outputs2))
