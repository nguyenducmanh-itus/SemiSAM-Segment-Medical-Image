...
# Tham số cho dữ liệu không nhãn
...
def get_current_consistency_weight(epoch):
    ...
    Sử dụng hàm sigmoid_rampup để tăng dần trọng số, thường thấy trong các phương pháp Mean Teacher.
    """
...
def train(args, snapshot_path):
    ...
    Args:
        ...
        snapshot_path: Đường dẫn thư mục để lưu các checkpoint mô hình.
    """
    ...
    ce_loss = CrossEntropyLoss()# Hàm mất mát Cross Entropy
    dice_loss = losses.DiceLoss(2)# Hàm mất mát Dice
    ...
    iterator = tqdm(range(max_epoch), ncols=70)# Vòng lặp epoch với thanh tiến trình
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # --- Forward pass ---
            outputs = model(volume_batch)# Đầu ra của mô hình học sinh
            outputs_soft = torch.softmax(outputs, dim=1)# Chuyển đầu ra sang xác suất mềm (softmax)
            with torch.no_grad():
                ema_output = ema_model(ema_inputs)# Đảm bảo không tính gradient cho mô hình EMA
                ema_output_soft = torch.softmax(ema_output, dim=1)# Đầu ra của mô hình giáo viên
            # --- Tính toán Loss ---
            # Loss có giám sát (chỉ trên dữ liệu được gán nhãn)
            loss_ce = ce_loss(outputs[:args.labeled_bs],# Cross Entropy loss
                              label_batch[:args.labeled_bs][:])
            loss_dice = dice_loss(# Dice loss
                outputs_soft[:args.labeled_bs], label_batch[:args.labeled_bs].unsqueeze(1))
            supervised_loss = 0.5 * (loss_dice + loss_ce)
             # Loss nhất quán cho Mean Teacher
            ...
            if args.prompt == 'unc':# Nếu prompt là 'uncertainty'
                ...
                # Đơn giản là MSE giữa đầu ra của mô hình và đầu ra của SAM
                sam_consistency = torch.mean(
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                # Kiểm tra hiệu suất Dice score trung bình
                if avg_metric[:, 0].mean() > best_performance:
                    best_performance = avg_metric[:, 0].mean()
                    # Lưu mô hình với tên có iter và dice score
                    save_mode_path = os.path.join(snapshot_path,
                                                  'iter_{}_dice_{}.pth'.format(
                                                      iter_num, round(best_performance, 4)))
                    # Lưu mô hình tốt nhất (ghi đè nếu có)
...
if __name__ == "__main__":
    ...
    # Tạo đường dẫn để lưu kết quả huấn luyện (model checkpoints, logs, code)
    snapshot_path = "../model/{}_{}/{}".format(
        args.exp, args.labeled_num, args.model)
    if not os.path.exists(snapshot_path):
        os.makedirs(snapshot_path)
     # Xóa và sao chép mã nguồn hiện tại vào thư mục snapshot_path/code
